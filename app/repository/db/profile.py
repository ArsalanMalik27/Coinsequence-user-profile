...
class UserProfileDBRepository(
    ...
    async def get_by_user_id(self, user_id: UUID) -> Optional[UserProfileProps]:
        return await self._get_profle_by_id_and_column(user_id, UserProfile.user_id)

    async def get_profile_by_id(self, id: UUID) -> Optional[UserProfileProps]:
        return await self._get_profle_by_id_and_column(id, UserProfile.id)
    ...
    async def get_all_by_user_ids(self, user_ids: list[UUID]) -> Page[UserProfileProps]:
        async with self._db_session() as session:
            ...
            return Page(items=items, page=1, page_size=len(items), total=len(items))

    async def _get_profle_by_id_and_column(self, id: UUID, column) -> Optional[UserProfileProps]:
        async with self._db_session() as session:
            query = self.select().where(column == id)
            ...
            profile_entity = self._entity.from_orm(obj)

            education_query = (
                select(Education)
                .where(
                    Education.is_current == True,
                    Education.profile_id == profile_entity.id,
                    Education.deleted != True,
                )
            )

            education_result = await session.execute(education_query)
            education = education_result.scalars().first()

            education = [EducationProps.from_orm(education)] if bool(education) else None
            profile_entity.educations = education
    ...
    async def paginate_users(
        ...
        async with self._db_session() as session:
            ...
            #     # .where(limit_query.c.id == UserProfile.id)
            # )

            paginated_query = (
                self.select()
                .where(UserProfile.id == query.c.id)
                .order_by(desc(UserProfile.created_at))
                .limit(page_size)
    ...
    async def get_searched_users(
        ...
        if text:
            ...
            text = '&'.join(text.split())
            query = query.where(UserProfile.__ts_vector__.match(f'{text}:*'))
    ...
    async def get_all_joined_by_user_id(self, user_id: UUID):
        async with self._db_session() as session:
            query = (
                self.select()
                .options(
                    joinedload(UserProfile.educations),
                    joinedload(UserProfile.activity),
                    joinedload(UserProfile.application),
                    joinedload(UserProfile.award),
                    joinedload(UserProfile.collegeUniversities),
                    joinedload(UserProfile.grade),
                    joinedload(UserProfile.profilePrivacy),
                    joinedload(UserProfile.roles),
                    # joinedload(UserProfile.studentFundCourses),
                    joinedload(UserProfile.test),
                    # contains_eager(UserProfile.userKarma),
                    joinedload(UserProfile.voluntary),
                    joinedload(UserProfile.work),
                )
                .where(UserProfile.user_id == user_id)
            )
            result = await session.execute(query)

            fund_courses_result = await session.execute((
                select(StudentFundCourses)
                .where(
                    StudentFundCourses.user_id == user_id,
                    StudentFundCourses.deleted is not True,
                )
            ))

            user_karma_result = await session.execute((
                select(UserKarma)
                .where(
                    UserKarma.user_id == user_id,
                    UserKarma.deleted is not True,
                )
            ))
            ...
            fund_courses_obj = fund_courses_result.scalars().all()
            user_karma_obj = user_karma_result.scalars().all()

            obj_dict = jsonable_encoder(obj)
            
            obj_dict["studentFundCourses"] = fund_courses_obj
            obj_dict["userKarma"] = user_karma_obj
